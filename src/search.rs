...
//!
//! Mirrors `python/src/memoro/search.py`: the tokenizer rules (NFKC + full
//! case folding, then non-CJK alphanumeric words and CJK character bigrams),
//! the BM25F constants, field weights, and scoring formula, and the snippet
//! windowing are ported verbatim.
...
pub const MAX_LIMIT: i64 = 8;
pub const DEFAULT_SNIPPET_BUDGET: usize = 700;
pub const DEFAULT_TOTAL_BUDGET: usize = 2400;
...
const SNIPPET_ALIGNMENT: usize = 80;
...
pub fn search_memories(
    ...
    project: Option<&str>,
    limit: i64,
) -> Result<SearchResult, MemoroError> {
    if query.trim().is_empty() {
        return Err(MemoroError::MemoryValidation(
            "Search query is empty. Provide non-blank text to find memories.".to_string(),
        ));
    }
    let limit_value = limit.clamp(1, MAX_LIMIT);
    ...
    let filtered: Vec<(&str, &Memory)> = entries
        .iter()
        .filter(|(_, memory)| {
            kind.is_none_or(|value| memory.kind == value)
                && project.is_none_or(|value| memory.project.as_deref() == Some(value))
        })
        .copied()
        .collect();
    if filtered.is_empty() || query_terms.is_empty() {
    ...
    for (space, memory) in &filtered {
        let (frequencies, length) = weighted_term_frequencies(memory);
        total_length += length;
    ...
    let average_length = total_length / documents.len() as f64;
    ...
    for term in &query_terms {
        let document_frequency = documents
            .iter()
            .filter(|document| document.frequencies.contains_key(term.as_str()))
            .count() as f64;
        let value = ((corpus_size - document_frequency + 0.5) / (document_frequency + 0.5)).ln();
        idf.insert(term.as_str(), value.max(0.0));
    }

    let mut matches: Vec<RankedMatch> = Vec::new();
    for document in &documents {
        let saturation = BM25_K1 * (1.0 - BM25_B + BM25_B * document.length / average_length);
        let mut score = 0.0;
        ...
        for term in &query_terms {
            let frequency = document
                ...
                .copied()
                .unwrap_or(0.0);
            if frequency <= 0.0 || idf[term.as_str()] <= 0.0 {
                continue;
            }
            let contribution =
                idf[term.as_str()] * frequency * (BM25_K1 + 1.0) / (frequency + saturation);
            score += contribution;
    ...
    for ranked in matches.iter().take(limit_value as usize) {
        let (snippet, snippet_truncated) =
            build_snippet(&ranked.memory.body, &ranked.contributions);
        let cost = ranked.memory.title.chars().count()
            + ranked.memory.summary.chars().count()
            + snippet.chars().count();
    ...
    let truncated = hits.len() < matches.len();
    Ok(SearchResult {
        hits,
...
fn is_cjk_character(character: char) -> bool {
    ...
    let index = CJK_RANGES.partition_point(|&(start, _)| start <= codepoint);
    index > 0 && codepoint <= CJK_RANGES[index - 1].1
}

fn flush_run(tokens: &mut Vec<String>, run: &mut String, run_is_cjk: bool) {
    if run.is_empty() {
        return;
    }
    if run_is_cjk && run.chars().count() > 1 {
...
fn weighted_term_frequencies(memory: &Memory) -> (HashMap<String, f64>, f64) {
    let mut frequencies: HashMap<String, f64> = HashMap::new();
    let mut length = 0.0;
    for (field, weight) in FIELD_WEIGHTS {
        let tokens = field_tokens(memory, field);
        length += weight * tokens.len() as f64;
        for token in tokens {
            *frequencies.entry(token).or_insert(0.0) += weight;
        }
    }
    (frequencies, length)
}

fn field_tokens(memory: &Memory, field: &str) -> Vec<String> {
    if field == "tags" {
        let mut tokens: Vec<String> = Vec::new();
    ...
    let value = match field {
        ...
        "body" => &memory.body,
        _ => unreachable!("FIELD_WEIGHTS only names Memory fields"),
    };
    tokenize(value)
}

fn build_snippet(body: &str, contributions: &[(String, f64)]) -> (String, bool) {
    let characters: Vec<char> = body.chars().collect();
    if characters.len() <= DEFAULT_SNIPPET_BUDGET {
        return (body.to_string(), false);
    }
    // Positions are located in the folded body; NFKC + casefold is
    // length-preserving for ASCII and CJK text, so the index maps back onto
    // the original body closely enough for a snippet window. The clamp
    // guards the rare length-changing folds.
    let folded = casefold(&nfkc(body));
    let mut anchor = 0;
    let mut best_rank: Option<(f64, i64)> = None;
    for (term, contribution) in contributions {
        let Some(position) = find_char_index(&folded, term) else {
            continue;
        };
        let rank = (*contribution, -(position as i64));
        if best_rank.is_none_or(|best| rank_greater(rank, best)) {
            best_rank = Some(rank);
            anchor = position.min(characters.len() - 1);
        }
    }
    let half_budget = DEFAULT_SNIPPET_BUDGET / 2;
    let start = ((anchor as i64 - half_budget as i64)
        .min(characters.len() as i64 - DEFAULT_SNIPPET_BUDGET as i64)
        .max(0)) as usize;
    let end = start + DEFAULT_SNIPPET_BUDGET;
    let (start, end) = align_to_lines(&characters, start, end);
    let prefix = if start > 0 { "…" } else { "" };
    let suffix = if end < characters.len() { "…" } else { "" };
    let window: String = characters[start..end].iter().collect();
    (format!("{prefix}{window}{suffix}"), true)
}

fn align_to_lines(body: &[char], start: usize, end: usize) -> (usize, usize) {
    let mut start = start;
    let mut end = end;
    if start > 0 && body[start - 1] != '\n' {
        let search_end = end.min(start + SNIPPET_ALIGNMENT);
        if let Some(offset) = body[start..search_end].iter().position(|&c| c == '\n') {
            start += offset + 1;
        }
    }
    if end < body.len() && body[end] != '\n' && body[end - 1] != '\n' {
        let search_start = start.max(end - SNIPPET_ALIGNMENT);
        if let Some(offset) = body[search_start..end].iter().rposition(|&c| c == '\n') {
...
/// Character (not byte) offset of the first `needle` occurrence, mirroring
/// Python's `str.find` indices.
...
/// Lexicographic `>` over the `(contribution, -position)` anchor rank.
fn rank_greater(rank: (f64, i64), best: (f64, i64)) -> bool {
    rank.0 > best.0 || (rank.0 == best.0 && rank.1 > best.1)
}

// ---------------------------------------------------------------------------
// Unicode folding for tokenization and snippet anchoring. The functions and
// tables below are copied verbatim from `models.rs` (generated from Python
// 3.13's `unicodedata`, the semantic reference); keep the copies in sync
// until the helpers are lifted into a shared module.
// ---------------------------------------------------------------------------
