...
class RepositoryLocks:
    def __init__(self, lock_dir: Path, *, timeout: int = LOCK_TIMEOUT_SECONDS) -> None:
        self.lock_dir = lock_dir
        self.repository_path = lock_dir / "repository.lock"
    ...
    def _acquire(self, path: Path, flag: portalocker.LockFlags) -> Iterator[None]:
        ...
        try:
            ...
            raise RepositoryError(
                f"Memoro could not acquire the local lock {path} within {self.timeout} seconds. "
