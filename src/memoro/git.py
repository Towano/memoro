...
from memoro.errors import RepositoryDirtyError, RepositoryError

GIT_IDENTITY_NAME = "Memoro"
GIT_IDENTITY_EMAIL = "memoro@localhost"
PUSH_TIMEOUT_SECONDS = 15
SYNC_CONFLICT_REF = "refs/memoro/sync-conflict"
MEMORY_TOP_LEVEL_DIRS = ("persona", "playbooks", "projects")
...
class GitRepository:
    ...
    def initialize(cls, path: Path) -> GitRepository:
        ...
        if resolved.exists() and not resolved.is_dir():
            raise RepositoryError(
                ...
                "Move that file or choose a different --home, then restart Memoro."
            )
    ...
    def open(cls, path: Path) -> GitRepository:
        ...
        if not resolved.is_dir():
            raise RepositoryError(
                f"Memory repository {resolved} does not exist. Start Memoro or run "
                "'memoro sync setup REPOSITORY_URL' first."
            )
        repo = cls(resolved)
        repo._validate_independent()
        return repo
    ...
    def _validate_independent(self) -> None:
        ...
        if (
            ...
            raise RepositoryError(
                ...
                "Memoro left its contents unchanged. Move them elsewhere or choose a "
    ...
    def assert_clean(self) -> None:
        ...
        if result.stdout:
            raise RepositoryDirtyError(
                f"Memory repository {self.path} has uncommitted changes. Memoro did not "
        ...
        if any(self._git_path(name).exists() for name in operation_paths):
            raise RepositoryDirtyError(
                f"Memory repository {self.path} has an unfinished Git operation. Memoro did "
    ...
    def memory_paths_at_commit(self, commit: str) -> list[str]:
        result = self._run_bytes(
            ["ls-tree", "-r", "-z", commit, "--", *MEMORY_TOP_LEVEL_DIRS]
        )
        paths = []
    ...
    def commit(self, message: str, relative_path: str) -> str:
        ...
        if staged != [relative_path]:
            raise RepositoryError(
                ...
                "Memoro stopped before committing; inspect the memory repository index."
            )
        with tempfile.TemporaryDirectory(prefix="memoro-hooks-") as hooks_path:
            self._run(
                [
                    "-c",
    ...
    def remote_url(self, remote: str) -> str | None:
        ...
        if len(urls) != 1 or len(push_urls) != 1 or urls != push_urls:
            raise RepositoryError(
                ...
                "Memoro left it unchanged; simplify that remote manually, then retry."
            )
        return urls[0]
    ...
    def fetch(self, remote: str, branch: str, timeout: int = PUSH_TIMEOUT_SECONDS) -> str | None:
        if self.remote_url(remote) is None:
            raise RepositoryError(
                ...
                "'memoro sync setup REPOSITORY_URL' in this Memoro home, then retry."
            )
        try:
            result = self._run(
    ...
    def configure_deploy_key(self, private_key: Path, known_hosts: Path) -> None:
        ...
        self._run(["config", "--local", "memoro.syncAuth", "deploy-key"])
        self._run(
            ["config", "--local", "memoro.deployKeyPath", os.fspath(private_key)]
        )

    def deploy_key_path(self) -> Path | None:
        auth = self._run(
            ["config", "--local", "--get", "memoro.syncAuth"],
            check=False,
        )
        if auth.returncode != 0 or auth.stdout.strip() != "deploy-key":
            return None
        result = self._run(
            ["config", "--local", "--get", "memoro.deployKeyPath"],
            check=False,
        )
        ...
        *,
        ...
        if "non-fast-forward" in output or "fetch first" in output:
            raise RepositoryError(
                ...
                f"{branch!r}. Memoro did not fetch, merge, or force-push it. Use an empty "
