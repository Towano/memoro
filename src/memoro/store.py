...
from memoro.markdown import memory_revision, parse_memory, serialize_memory
...
class MemoryStore:
    ...
    def create(
        ...
        *,
        ...
        normalized_kind, normalized_project = _validated_location(kind, project)
        normalized_tags = _validated_tags(tags)
        label = location_label(normalized_kind, normalized_project)

        snapshot = self._writable_snapshot()
        matching = [
            memory
            for memory in snapshot.memories
            if memory.kind == normalized_kind
            and memory.project == normalized_project
            and title_key(memory.title) == title_key(normalized_title)
        ]
        if matching:
            existing = matching[0]
            if (
                ...
                and existing.tags == normalized_tags
            ):
                return _unchanged_receipt("create", existing, snapshot)
            raise MemoryConflictError(
                f"Memory {existing.id} already uses this title in {label}. Read it and use patch "
        ...
        memory = Memory(
            ...
            relative_path=memory_path(memory_id, normalized_kind, normalized_project),
        )
    ...
    def replace(
        ...
        *,
        ...
        _require_revision(previous, normalized_revision)
        if (
            normalized_summary == previous.summary
            and normalized_body == previous.body
            and normalized_tags == previous.tags
        ):
        ...
        *,
        ...
        try:
            ...
            if current_head != snapshot.commit:
                raise RepositoryError(
                    "The Git commit command failed after HEAD changed. Memoro did not alter the "
    ...
    def _validate_integrity(memories: tuple[Memory, ...]) -> None:
        ids: dict[str, str] = {}
        titles: dict[tuple[str, str | None, str], str] = {}
        for memory in memories:
            ...
            key = (memory.kind, memory.project, title_key(memory.title))
            if key in titles:
                label = location_label(memory.kind, memory.project)
                raise MemoryIntegrityError(
                    f"Kind {label!r} has duplicate normalized titles in "
...
def _validated_location(kind: str, project: str | None) -> tuple[str, str | None]:
    try:
        return validate_location(kind, project)
    except (TypeError, ValueError) as exc:
        raise MemoryValidationError(
            f"Memory location is invalid: {exc}. Use kind persona, project, or playbook, and "
...
def _validated_tags(tags: Sequence[str]) -> tuple[str, ...]:
    try:
        ...
        raise MemoryValidationError(
            f"Memory tags are invalid: {exc}. Use at most {MAX_TAGS} non-empty plain-text tags "
            "of at most 32 characters each."
        ) from exc
...
def _commit_message(operation: str, memory: Memory) -> str:
    ...
    return f'memory({label}): {operation} "{safe_title}"'
